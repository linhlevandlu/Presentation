...
\definecolor{pool}{RGB}{255, 210, 0}
\definecolor{drop}{RGB}{153, 255, 102}
...
%-------------------------------------------------------
	\begin{block}{Manual landmarks}		
		\begin{itemize}
	  				\item Time-consuming
					\item Difficult to pre-procedure
  				\end{itemize}
	\end{block}
...
\begin{frame}{Our proposed architecture}{Elementary block}
  \begin{columns}
		\begin{column}{0.45\textwidth}	
			...
			\textbf{\color{red}Elementary block}: 
			\small{
			\begin{itemize}
				\only<1,2>{\item A \textbf{\color{conv}convolutional} layer,
    			\item A \textbf{\color{pool}maxixum pooling} layer,}
    			\only<2>{\item A \textbf{\color{drop}dropout} layer}
  			\end{itemize}  			
  			}			 			
		\end{column}
...
\begin{frame}{Our proposed architecture}{Elementary blocks composition}
	The proposed model: 
		\small{
			\begin{itemize}
				\item \textbf{Three} \textbf{\color{red}elementary blocks},
...
\begin{frame}{Fine-tuning our model}{}
	\only<1-2>{	
	\begin{itemize}
		\item Estimated landmarks on pronotum images when fine-tuning on \textbf{VGG-16, VGG-19, ResNet50} has not been improved
